...
int pgw_config_process (pgw_config_t * config_pP)
{
  ...
  for (int i = 0; i < config_pP->num_ue_pool; i++) {
    ...
    // struct in_addr wifi_addr;

    // if (inet_pton (AF_INET, bdata(bfromcstr("10.42.0.88")), buf_in_addr) == 1) {
    //   memcpy (&wifi_addr, buf_in_addr, sizeof (struct in_addr));
    // }

    // ip4_ref = calloc (1, sizeof (conf_ipv4_list_elm_t));
    // ip4_ref->addr = wifi_addr;
